...
- `/clear` — очистить историю (системный промпт сохранится, если задан),
- `/history` — показать текущие сообщения/резюме в памяти.

Каждые 10 пользовательских сообщений история автоматически ужимается в краткое резюме, чтобы экономить контекст.
